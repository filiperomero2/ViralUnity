...
PLACEHOLDER_SCRIPT = os.path.join(os.getcwd(), "test", "dryrun_configs", "create_dryrun_placeholders.sh")
...
def setup_dryrun_placeholders():
    ...
    if os.path.exists(PLACEHOLDER_SCRIPT):
        ...
        result = subprocess.run(["bash", PLACEHOLDER_SCRIPT], check=True, capture_output=True, text=True)
        print(result.stdout)
    else:
...
def test_snakemake_dryrun(config_filename):
    ...
    assert result.returncode == 0, f"Snakemake dry-run failed for {config_filename}\nSTDOUT: {result.stdout}\nSTDERR: {result.stderr}"
