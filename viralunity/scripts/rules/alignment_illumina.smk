...
rule trim_primer_sequences:
    ...
    log:
        config['output'] + f"assembly/{SEGMENT_WILDCARD}logs/samtools/ampliconclip/{{sample}}.log"
    benchmark:
        config['output'] + f"assembly/{SEGMENT_WILDCARD}logs/samtools/ampliconclip/{{sample}}.benchmark.txt"
    threads: config["threads"]
    shell:
        """
