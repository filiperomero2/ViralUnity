...
def run_snakemake():
    ...
    sample_to_fastq = getattr(snakemake.params, "sample_to_fastq", None)
    if sample_to_fastq is None:
        sample_to_fastq = getattr(snakemake.params, "samples", None)
    if sample_to_fastq is None:
        raise ValueError(
            "Snakemake params must include 'sample_to_fastq' or 'samples' "
            "(sample -> path to FASTQ for read count)."
        )
